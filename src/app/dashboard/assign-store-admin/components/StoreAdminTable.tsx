...
export default function StoreAdminData({
  ...
  const storeAdminData = allStoreAdmins.map((admin) => {
    const store = storeAdmins.find((s) => s.id === admin.store_id);
    return {
      adminId: admin.id,
      adminFirstName: admin.first_name,
      adminLastName: admin.last_name,
      storeName: store ? store.name : "NO STORE",
  ...
  // revert store admin to customer
    ...
    <div>
      <Card className={`${className}`}>
        <CardHeader className="flex flex-col">
          ...
          <div id="searcbar" className="relative w-full mt-4">
            <Input
              value={searchQuery}
            ...
            <TableBody className="text-center">
              {currentUsers.map((admin, idx) => (
                <TableRow key={idx}>
                  ...
                  <TableCell
                    className={
                      admin.storeName === "NO STORE" ? "text-red-500" : ""
                    }
                  >
                    {admin.storeName}
                  </TableCell>
